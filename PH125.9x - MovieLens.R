...
# (note) n_distinct(): efficiently count the number of unique values in a set of vector
...
edx <- edx %>%
  ...
  # (note) dylyr/extract(): Extract a character column into multiple columns using regular expression groups
...
# (note) kable(): Create tables in LaTeX, HTML, Markdown and reStructuredText
# (note) the kable() function in knitr is a very simple table generator, and is simple by design. 
...
train_set %>% 
  group_by(userId) %>% 
  # (note) note you can get number of items in each group by directly using n(). Very useful. 
  filter(n()>=100) %>%
